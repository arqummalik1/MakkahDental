...
export function PatientLoginClient(props: { nextPath: string }) {
  const router = useRouter();
  const next = props.nextPath || "/patient/dashboard";
    ...
    <div className="grid min-h-dvh place-items-center bg-[var(--color-bg)] px-4">
      <div className="mdc-neo w-full max-w-md rounded-[var(--radius-lg)] p-8">
        ...
        >
          <div>
            ...
            {form.formState.errors.email?.message ? (
              <p className="mt-2 text-base text-rose-700">{form.formState.errors.email?.message}</p>
            ) : null}
          </div>
          ...
          <div>
            ...
            {form.formState.errors.password?.message ? (
              <p className="mt-2 text-base text-rose-700">{form.formState.errors.password?.message}</p>
            ) : null}
          </div>
          ...
          <div className="text-center text-base text-[var(--color-muted)]">
            New patient? <span className="text-[var(--color-fg)]">Registration is demo-only.</span>
          </div>

          <div className="text-center text-base">
            <Link className="mdc-animate text-[var(--color-primary)] hover:underline" href="/admin/login">
              Admin Login →
            </Link>
          </div>
        </form>
        ...
        <div className="mt-8 rounded-[var(--radius-md)] bg-[var(--gray-100)] px-4 py-4 text-base text-[var(--color-muted)]">
          Demo credentials: patient@demo.com / patient123
        </div>
      </div>
    </div>
  );
